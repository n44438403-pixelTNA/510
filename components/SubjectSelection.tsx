    ...
    <div className="animate-in fade-in slide-in-from-right-8 duration-500">
      {(!hideBack || selectedParentSubject) && (
        <div className="flex items-center mb-8">
            <button onClick={handleBack} className="text-slate-500 hover:text-slate-800 transition-colors mr-4">
            &larr; Back
            </button>
            <div>
            ...
            <p className="text-slate-500 text-sm">Select a {selectedParentSubject ? 'sub-subject' : 'subject'} to view chapters</p>
            </div>
        </div>
      )}
        ...
        {activeList.map((subject) => (
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{selectedParentSubject || subject.name === 'Science' || subject.name === 'Social Science' ? 'Explore Sections' : 'Explore Syllabus'}</p>
            </div>
          </button>
        ))}
      </div>
