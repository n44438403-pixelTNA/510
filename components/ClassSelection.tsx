    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-4xl mx-auto px-4">
        {classes.map((cls) => {
            ...
            // ACCOUNT LOCK (1 Account = 1 Class)
            if (userLockedClass && userLockedClass !== cls) {
                isLocked = true;
                lockReason = "Account Locked";
            }
                ...
                {isLocked && (
                    <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-20 flex flex-col items-center justify-center text-center p-2 animate-in fade-in">
                        ...
                        <span className="text-xs font-black text-slate-500 uppercase tracking-wider">
                            {userLockedClass && userLockedClass !== cls ? "LOCKED" : "Coming Soon"}
                        </span>
                        <span className="text-[9px] text-slate-400 mt-1 font-medium">
                            {userLockedClass && userLockedClass !== cls
