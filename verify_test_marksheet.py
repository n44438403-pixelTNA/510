...
def run(playwright):
    ...
    # 4. Click on the history item to open the marksheet from Student Dashboard -> REVISION Hub
    try:
        # First click on Revision Hub Tab if not there (bottom nav)
        try:
            page.locator("button:has-text('Hub')").click(timeout=2000)
            time.sleep(2)
        except:
            print("Could not find Hub tab button")
            pass

        # Then find the card with text "Chemical Reactions"
        ...
        page.locator("details summary").first.click(timeout=2000)
        time.sleep(1)

    except Exception as e:
        print(f"Error navigating to marksheet: {e}")

    page.screenshot(path="student_marksheet_redesign2.png", full_page=True)
    browser.close()
